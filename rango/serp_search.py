...
# use SERP scale search api instead bing
def read_bing_key():
...
def run_query(search_terms):
    ...
    params = {'api_key':bing_key, 'q': search_terms}
    ...
    results = []
    # for result in search_results['organic_results']['value']:
        # results.append({'title': result['name'], 'link': result['url'], 'summary': result['snippet']})
    for result in search_results['organic_results']:
        results.append({'title': result['title'], 'link': result['link'], 'summary': result['snippet']})

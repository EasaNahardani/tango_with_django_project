...
from rango.forms import  UserProfileForm, CategoryForm, PageForm,UserForm
...
##   ویو های مبنی بر فامکشن


############### در صورتی که در سمت کلاینت اطلاعات ذخیره شوند
# def visitor_cookie_handler(request, response):
    ...
    # if (datetime.now() - last_visit_time).days > 0:
        # visits = visits + 1
        # print( f"if executed-visits:{visits}")
...
def about(request):
    ############### اگر دیکشنری نیاز نداشتیم
    # or  return render(request, 'rango/about.html', {})
    #################### تست کوکی
...
def index(request):
    ...
    ############ در صورتی که  اطلاعات در کوکی ها سمت کلاینت ذخیره شود
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if category:
                ...
                page.views = 0
                page.save()
                # or  return redirect( 'rango:show_category', category_name_slug = category_name_slug )
                return redirect(reverse('rango:show_category',kwargs={'category_name_slug': category_name_slug}))
        else:
            print(form.errors)
    context_dict = {'form': form, 'category': category}
...
def search(request):
    ...
    if request.method == 'POST':
        query = request.POST['query'].strip()
        # or  query = request.POST.get('query').strip()
        if query:
            # Run our Bing function to get the results list!
...
def old_goto_url(request):
    ...
    try:
        ...
        return redirect(reverse('rango:index'))
    # در صورت بازدید پیج باید ویو افزایش یابد
    # پس باید افزایش ویو بعد از کد
    # except
    # نوشته شود
    selected_page.views = selected_page.views + 1
    selected_page.save()
